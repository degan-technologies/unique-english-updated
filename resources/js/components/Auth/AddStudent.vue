        ...
        const submitForm = async () => {
            ...
            try {
            const response = await Axios.post("/api/add-student", form.value);
        ...
        return { form, errors, loading, message, errorMessage, submitForm,frontLang };
        },
        };
        </script>
        <template>
        <div class="flex justify-center items-center min-h-screen bg-white-to-br from-white-100 to-white-300 px-4">
        <div class="w-full max-w-lg bg-white p-6 rounded-2xl shadow-xl">
            <!-- Header -->
            <h6 class="text-xl font-extrabold text-gray-800 text-center mb-6">{{frontLang.lang.register_student}}</h6>
