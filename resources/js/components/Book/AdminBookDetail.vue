...
<template>
    <div class="relative">
        <div v-if="!isReadingMode" class="w-full mx-auto sm:px-6 lg:px-8 py-8">
            <button
                @click="goBack"
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-8">
                <div class="rounded-lg sm:p-6 flex flex-col gap-6 w-full">
                    <div class="flex flex-col lg:flex-row w-full mx-auto gap-6">
                        <div
                            class="relative rounded-lg flex-grow overflow-hidden bg-gray-100 aspect-video"
                        >
                            <template
                                v-if="
                                    selectedbook?.intro_video_url &&
                                    selectedbook?.video_optimized
                                "
                            >
                                <VueVideoPlayer
                        ...
                        >
                            ...
                            <div class="flex flex-col gap-4">
                                <div
                                    v-for="(value, key) in bookDetails"
                                    :key="key"
                                    class="flex items-start gap-3"
                                >
                                    <i
                                        class="fas fa-circle-check text-lime-600 text-sm mt-1.5"
                                    ></i>
                                    <p
                                        :class="[
                                    ...
                                    >
                                        <span
                                            class="font-semibold pr-2 text-gray-800"
                    ...
                    <div
                        class="p-0 sm:p-4 pt-8 rounded-b-lg bg-white shadow-sm"
                    >
                        <h2
                            class="text-2xl sm:text-3xl text-slate-700 font-semibold mb-4 sm:mb-6"
                        >
                            {{ selectedbook?.title }}
                        </h2>
                        <div
                            class="prose prose-sm sm:prose-base max-w-none ql-editor preview"
