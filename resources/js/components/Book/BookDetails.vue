<template>
  <div>
    <!-- Conditionally render PDF Viewer if showPdfViewer is true -->
    <div v-if="showPdfViewer">
      <PdfViewer :pdfUrl="selectedbook?.file_url" />
      <!-- Optionally, add a button to exit the PDF viewer -->
      <button @click="showPdfViewer = false" class="mt-4 bg-red-500 text-white px-4 py-2 rounded">
        Close PDF Viewer
      </button>
    </div>

    <!-- Original content (displayed when PDF viewer is not active) -->
...
import PdfViewer from "./Pdf.vue"; // Adjust the path as needed
