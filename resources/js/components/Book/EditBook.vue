...
// On mount or when selected.tag changes, parse it
onMounted(() => {
        ...
        <form class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <label class="block text-gray-700 font-medium text-xs mb-1 mt-2">Upload Cover Page</label>
                    <div class="relative border border-2 border-gray-300 rounded text-center cursor-pointer">
                        <span class="text-gray-400 text-sm">Click to select cover image</span>
                        <input type="file"
                ...
                <div>
                    ...
                    <div v-else-if="selected.intro_vedio && typeof selected.intro_vedio === 'string'">
                        <p class="text-xs text-gray-700">
                            Current Video:
                            <a :href="selected.intro_vedio"
                                target="_blank"
                                class="underline">{{ selected.intro_vedio.split('/').pop() }}</a>
                        </p>
                    </div>
                    <label class="block text-gray-700 font-medium text-xs mb-1 mt-2">Upload Your Intro Video</label>
                    <div class="relative border border-2 border-gray-300 rounded text-center cursor-pointer ">
                        <span class="text-gray-400 text-sm">Click to select intro video</span>
                        <input type="file"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div><label class="label">Title</label><input v-model="selected.title"
                        type="text"
                        class="input"
                        required />
                </div>
                <div><label class="label">Author</label><input v-model="selected.auther"
                        type="text"
                        class="input"
                        required />
                </div>
                <div><label class="label">Price</label><input v-model.number="selected.price"
                        type="number"
                        class="input"
                        required />
                </div>
                <div><label class="label">Edition</label><input v-model.number="selected.eddition"
                        type="number"
                        class="input"
                        required />
                </div>
                <div><label class="label">Discount</label><input v-model.number="selected.discount"
                        type="number"
                        class="input" />
                </div>
                <div><label class="label">Publish Date</label><input v-model="selected.publish_date"
                        type="date"
                        class="input"
                        required />
                </div>
                <div><label class="label">Language</label><input v-model="selected.language"
                        type="text"
                        class="input"
                        required />
                </div>
            ...
            <div class="flex gap-4 mt-4">
                <button type="button"
                    ...
                    class="w-full bg-green-500 text-white px-6 py-2 rounded hover:bg-green-600">Update Book</button>
                <button type="button"
                    @click="cancelEdit"
                    class="w-full bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600">Cancel</button>
            </div>
        </form>
    </div>
</template>
