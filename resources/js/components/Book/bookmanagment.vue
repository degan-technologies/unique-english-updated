...
.editor-wrapper {
  margin-bottom: 1.5rem; /* Increase bottom margin to ensure space for buttons */
  position: relative;
  z-index: 0;
}
</style>
...
<template>
  <div class="max-w-full mx-auto">
    ...
    <div class="flex justify-end mb-4">
      <button @click="toggleView" class="bg-lime-700 hover:bg-lime-900 text-white p-2 rounded-full">
        <i :class="computedViewMode === 'card' ? 'fa-solid fa-list' : 'fa-solid fa-th-large'" class="text-xl"></i>
      </button>
    </div>
    ...
    <div v-if="!selectedbook && !showPopularBooks">
      ...
      <div v-if="computedViewMode === 'card'">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-2 p-3">
          <div
            v-for="book in filteredBooksAdmin"
            :key="book.id"
          ...
          >
            ...
            <div class="mt-3 flex justify-around">
              <button @click="selectBookforEdit(book)" class="text-green-600 hover:underline text-sm">
                Edit
              </button>
              <button @click="changeTab(book.slug)" class="text-blue-600 hover:underline text-sm">
                Details
              </button>
              <button @click="deleteCourse(book.id)" class="text-red-600 hover:underline text-sm">
                Delete
              </button>
            </div>
          </div>
      ...
      <div v-if="computedViewMode === 'table'">
              ...
              >
                ...
                <td class="py-2 px-2 flex items-center gap-3">
                  ...
                  <div class="bg-white border border-gray-100 rounded-md p-1 relative">
                    ...
                    >
                      <button
                        @click="selectBookforEdit(book); closeMenu()"
        ...
        <!-- Pagination Controls -->
        <div v-if="pagination.last_page > 1" class="pagination flex justify-between items-center mt-4">
          <button
    ...
    <!-- Edit Book Form -->
    <div v-if="editingBookId" class="max-w-4xl mx-auto p-4 mt-6 bg-white shadow-lg rounded-lg border border-gray-200">
      ...
      <div>
        ...
        <div class="relative border-dashed border-2 border-gray-300 rounded p-2 text-center cursor-pointer">
          ...
          <div v-if="selectedbook && selectedbook.cover_page_url">
            <img
              :src="typeof selectedbook.cover_page_url === 'string' ? selectedbook.cover_page_url : URL.createObjectURL(selectedbook.cover_page_url)"
              alt="Cover Preview"
 ...
 <div>
  ...
  <div class="relative border-dashed border-2 border-gray-300 rounded p-2 text-center cursor-pointer">
    ...
    <div v-if="introVideoPreview">
      <video controls class="w-full max-h-40 mx-auto">
    ...
    <div class="mt-7">
      ...
      <textarea
        ...
        style="height:150px;"
        class="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-lime-500"
      ></textarea>
      <p v-if="!selectedbook.description" class="text-red-500 text-xs mt-1">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
      ...
      <div>
        ...
        <input
          v-model.number="selectedbook.eddition"
          type="number"
          placeholder="e.g. 1"
          class="w-full border border-gray-300 rounded-sm p-1 text-xs"
          required
      ...
      <div>
        ...
        <input
          v-model.number="selectedbook.discount"
          type="number"
      ...
      <div>
        ...
        <input
          v-model="selectedbook.publish_date"
          type="date"
          class="w-full border border-gray-300 rounded-sm p-1 text-xs"
          required
        />
      ...
      <div>
        ...
        <input
          v-model.number="selectedbook.page_number"
          type="number"
      ...
      <div>
        ...
        <select
          v-model="selectedbook.file_format"
          class="w-full border border-gray-300 rounded-sm p-1 text-xs"
          required
        >
    ...
    <div class="flex gap-4 mt-4">
      <button type="submit" class="w-full bg-green-500 text-white px-6 py-2 rounded hover:bg-green-600">
        Update Book
      </button>
      <button type="button" @click="cancelEdit" class="w-full bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600">
        Cancel
      </button>
    </div>
  </form>
    ...
    <!-- Book Detail Page -->
    <div v-if="showPopularBooks && selectedbook" class="p-6 mt-24 pb-16 rounded-lg bg-slate-50">
      <div class="grid w-[90%] mx-auto grid-cols-1 md:grid-cols-[2fr_1fr] gap-8 relative">
        ...
        <div class="w-full border border-e-gray-300 h-1/2 min-h-fit rounded-lg p-6">
          ...
          <div class="flex flex-row gap-4 my-2">
            ...
            <p class="text-gray-600 text-lg"><span class="font-bold pr-4">Auther:</span>{{ selectedbook?.auther }}</p>
          </div>
          <div class="flex flex-row gap-4 my-2">
            <i class="fa-solid self-center fa-check text-lime-700 text-lg"></i>
          ...
          <div class="flex flex-row gap-4 my-2">
            <i class="fa-solid self-center fa-check text-lime-700 text-lg"></i>
            <p class="text-gray-600 text-lg"><span class="font-bold pr-4">Eddition:</span>{{ selectedbook?.eddition }} Birr</p>
          </div>
