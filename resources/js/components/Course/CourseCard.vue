    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
        <div v-for="course in courses" :key="course.id" class="relative p-4 bg-white rounded-lg shadow hover:shadow-lg transform transition-transform duration-300 hover:scale-105 cursor-pointer group">
            <div class="relative group">
                <img :src="course?.thumbnail_url" :alt="course.course_name" class="w-full h-40 object-cover rounded-t-lg" />
            ...
            <div class="p-4 flex flex-col">
                ...
                <div class="mt-6 text-lg font-semibold text-blue-500 flex items-center">
                    ...
                    <button @click="addItems(course)" class="ml-auto focus:outline-none">
                         <i class="fa-solid fa-cart-plus mb-2 material-icons absolute bottom-20 right-8 text-white rounded-full bg-blue-500 p-3 hover:text-lime-700"></i>
                    </button>
                </div>
                <button 
                    @click="changeTab(course.slug)"
                    class="mt-4 border border-lime-700 text-lime-800 px-4 py-2 rounded hover:bg-lime-800 hover:text-white focus:outline-none transition-colors font-medium text-lg">
                    Get Started
                </button>
            </div>
        </div>
