        ...
        <div class="grid gap-4 grid-cols-1 md:grid-cols-2 w-full md:w-3/4 mx-auto">
            <div class="w-full p-6 flex flex-col items-start relative">
                <div class="overflow-hidden w-full aspect-video rounded-t-lg mt-3 relative cursor-pointer">
                    <div v-if="!isPlaying" class="relative w-full h-full cursor-pointer" @click="isPlaying = true">
                        <img 
                    ...
                    <div v-else class="relative w-full h-full">
                        <iframe v-if="selectedCourse.intro_video.includes('youtube.com') || selectedCourse.intro_video.includes('youtu.be')" 
                        :src="selectedCourse.intro_video + '?autoplay=1'" 
                        class="w-full h-full rounded-t-lg shadow-md border" 
                        frameborder="0" allowfullscreen>
                        </iframe>

                        <video v-else controls autoplay class="w-full h-full rounded-t-lg shadow-md border">
                        <source :src="selectedCourse.intro_video" type="video/mp4">
        ...
        <div class="w-full md:w-3/4 mx-auto p-3 ">
            <div class="flex flex-col md:flex-row items-center justify-start">
                ...
                <div class="flex items-center space-x-4 ml-2">
                    <button 
                        @click="getSelectedCourse(courseEditTab, selectedCourse)" 
                        class="flex items-center space-x-1 px-4 py-2 bg-lime-100 text-lime-600 rounded-md hover:bg-lime-200 transition">
                        <i class="fas fa-edit"></i>
                        <span>Edit</span>
                    </button>
                    <button 
                        @click="openModuleForm(selectedCourse?.id)" 
                        class="flex items-center space-x-1 px-4 py-2 bg-purple-50 text-purple-600 rounded-md hover:bg-purple-100 transition">
                        <i class="fas fa-plus"></i>
                        <span>Add Module</span>
                    </button>
                    <button 
                        @click="deleteCourse(selectedCourse?.id)" 
                        class="flex items-center space-x-1 px-4 py-2 bg-slate-50 text-black rounded-md hover:bg-slate-100 transition">
