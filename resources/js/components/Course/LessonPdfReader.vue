<script setup>
    ...
    const loadPdf = async () => {
        if (!props.selectedLesson || !props.selectedLesson.content_url) return;
        try {
            pdfDoc = await getDocument({
                url: props.selectedLesson.content_url
            }).promise;
    ...
    onMounted(() => {
        if (props.selectedLesson && props.selectedLesson.content_type === 2) {
            loadPdf();
        }
    ...
    watch(() => props.selectedLesson, (newVal) => {
        if (newVal && newVal.content_type === 2) {
            loadPdf();
        }
    }, { immediate: true });
</script>
