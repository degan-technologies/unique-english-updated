<template>
    <div
        class="mt-2 w-full lg:w-full mx-auto bg-white p-4 rounded-lg shadow-lg"
    >
        <!-- Ask a Question -->
        <div class="mb-8">
            <input
                v-model="newQuestion"
                type="text"
                class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-lime-400 mb-4"
                placeholder="Ask a question..."
            />
            <button
                @click="addQuestion"
                class="bg-lime-500 text-white px-6 py-3 rounded-lg shadow hover:bg-lime-600 w-full lg:w-auto"
            >
                <i class="fas fa-plus mr-2"></i> Submit Question
            </button>
            <p v-if="questionError" class="text-red-500 text-sm mt-2">
                {{ questionError }}
            </p>
        </div>
        ...
        >
            ...
            >
                <div>
                    ...
                    <span v-if="!qa.isEditing"
                        >{{ qa.user }}: {{ qa.question }}</span
                    >
                    <input
                        v-else
                ...
                <div class="flex space-x-2">
                    <button @click="editQuestion(index)" class="text-blue-500">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button @click="deleteQuestion(index)" class="text-red-500">
            ...
            <!-- Answers -->
            <div v-if="qa.answers.length > 0" class="ml-4 mt-3 space-y-2">
                <div
                ...
                >
                    <div class="flex items-center">
                        ...
                        <div
                            ...
                            class="bg-lime-100 text-gray-800 p-3 rounded-lg"
                        >
                            {{ answer.user }}: {{ answer.text }}
                        </div>
                        <input
                            v-else
                            v-model="answer.editedText"
                            class="p-2 border rounded w-3/4"
                        />
                    </div>
                    <div class="flex space-x-2">
            ...
            <div class="mt-4 flex items-start space-x-2">
                <button
                    ...
                    class="text-lime-500 bg-lime-100 px-4 py-2 rounded-full"
                >
                ...
                <div v-if="qa.showReplyField" class="flex-1">
                    ...
                    <button
                        ...
                        class="bg-lime-500 text-white px-6 py-3 rounded-lg shadow mt-3 w-full"
                    >
                        <i class="fas fa-paper-plane mr-2"></i> Submit Answer
                    </button>
                    <p v-if="qa.answerError" class="text-red-500 text-sm mt-2">
                        {{ qa.answerError }}
                    </p>
                </div>
            </div>
        ...
        <div class="mt-4 text-center">
            <button
                ...
                class="bg-gray-200 text-lime-500 px-6 py-3 rounded-lg"
            >
                <i class="fas fa-chevron-down mr-2"></i> See More
            </button>
            <button
                v-if="visibleQuestions.length > 2"
