...
// Watch for changes in quizData and extract questions
watch(
  () => props.quizData,
  (newQuizData) => {
    if (newQuizData && newQuizData.questions) {
      ...
      examData.value = newQuizData;  // examData should include meta info like id, course_module_id, course_id, etc.
      loading.value = false;
    } else {
...
const storeResult = async () => {
  try {
    const payload = {
      ...
      quiz_id: examData.value?.id,
      course_module_id: examData.value?.course_module_id,
      course_id: examData.value?.course_id
    };
