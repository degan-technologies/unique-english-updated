      ...
      <div class="flex flex-col md:flex-row md:items-center md:space-x-6">
        ...
        <div class="flex-1 space-y-2">
          ...
          >
            ...
            <!-- count of feedBacks for that star -->
    ...
    <div class="bg-white shadow-lg rounded-lg p-6 space-y-4">
      <div class="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-2 md:space-y-0">
        <input
          ...
          class="flex-1 border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-lime-300"
        />
        <select
          class="border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-lime-300"
        >
...
const likeComment = async (index) => {
  ...
  if (interaction.liked) {
    // Toggle off like
    try {
      const response = await axios.post(`/api/feedbacks/${feedback.id}/like`, { toggle: true });
      feedback.likes = response.data.likes;
      feedbackStore.toggleLike(feedback.id);
    } catch (error) {
    ...
    // Add like
    try {
      const response = await axios.post(`/api/feedbacks/${feedback.id}/like`);
      feedback.likes = response.data.likes;
      // If previously disliked, adjust dislike count
      if (interaction.disliked) {
        feedback.dislikes = Math.max(feedback.dislikes - 1, 0);
      }
      feedbackStore.toggleLike(feedback.id);
    } catch (error) {
...
const dislikeComment = async (index) => {
  ...
  if (interaction.disliked) {
    ...
    try {
      ...
      feedback.dislikes = response.data.dislikes;
      // If previously liked, adjust like count
      if (interaction.liked) {
