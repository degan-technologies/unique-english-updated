...
const editRoom = async () => {
    try {
        const response = await Axios.put(`/api/rooms/${selectedRoom.value.id}`, {
            class_name: selectedRoom.value.class_name,
        })
        const index = rooms.value.findIndex(room => room.id === selectedRoom.value.id)
    ...
    <div class="p-4">
        ...
        <div class="mb-4 flex justify-end">
            <button @click="showAddModal = true"
                class="bg-lime-600 text-white px-4 py-2 rounded-lg hover:bg-lime-700 transition-colors">
                Add Room
            </button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="room in rooms" :key="room.id"
                ...
                <div class="flex justify-between items-start mb-3">
                    <h3 class="text-xl font-semibold text-gray-800">{{ room.class_name }}</h3>
                            ...
                            <template #content>
                                <div class="bg-gray-50 text-black w-48 shadow-lg rounded p-2">
                                    ...
                                    <div 
                                        ...
                                        :key="instructor.id"
                                        @click="assignInstructor(room.id, instructor.id)"
                ...
                <p class="text-gray-600 mb-4">{{ room.instructor_name }} {{room.middle_name}}</p>
                <div class="flex justify-between items-center text-sm text-gray-500 mb-4">
                    <span><i class="fas fa-users mr-2"></i>{{ room.participants || 0 }}</span>
                    <span><i class="fas fa-clock mr-2"></i>{{ room.duration }} min</span>
                </div>
                <div class="flex flex-col gap-2">
                    <div v-if="room.instructor" class="text-sm text-gray-600">
                        <i class="fas fa-chalkboard-teacher mr-2"></i>
                        Instructor: {{ room.instructor.first_name }} {{ room.instructor.last_name }}
                    </div>
                    ...
                    <tr v-for="user in users" 
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"> 
                            <div class="text-sm text-lime-700 flex flex-1 justify-between">
                                <p> {{ user.class_name }} </p> 
                             <Popper>
                                <i class="fa-solid fa-chevron-down text-lg font-bold w-6 h-6 p-1 rounded-full hover:bg-slate-200 "></i>
                                <template #content>
                                    <div class="bg-gray-50 text-black w-48 shadow-lg rounded p-2">
                                        <div v-if="true">
                                            <div class="py-2 border-b-2 border-gray-500">
                                            ...
                                            <div 
                                                v-for="room in rooms" 
                                                :key="room.id"
                                                @click.stop="assignClass(user.id, room.id)"
                                                class="block hover:bg-gray-200 text-sm  text-left gap-2">
                                                <span class="block px-4 py-2 hover:bg-gray-200">{{ room.class_name }}</span>
                                            </div> 
                                        </div>
                                        <div v-else>
                                            <div class="py-2 border-b-2 border-gray-500">
