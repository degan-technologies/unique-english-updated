...
function getMySchedules() {
    Axios.get("/api/my-schedule").then((res) => {
        sessions.value = res.data.data;
    });
}

function getMyParticipants() {
    Axios.get("/api/my-participants").then((res) => {
...
<template>
    <div class="min-h-screen bg-gray-100">
        <header
            class="bg-white shadow p-4 flex flex-col md:flex-row justify-between items-center"
        >
            <div>
                <h1 class="text-2xl font-bold text-gray-800">
                    Live Sessions & Student Engagement
                </h1>
                <p class="text-sm text-gray-600">
                    Monitor, host, and engage in live learning sessions
                </p>
            </div>
        </header>
        ...
        <div class="my-12">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
                        ...
                        >
                            <div class="flex justify-between items-center">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <span
                                        :class="session?.color"
                                        class="px-2 py-1 text-xs capitalize rounded"
                                    >
                                        {{ session.status }}
                                    </span>
                                </div>
                            ...
                            <div class="mt-2 flex justify-between items-center">
                                <button
                                    class="bg-lime-600 text-white px-3 py-1 rounded hover:bg-lime-700 transition-colors"
                ...
                <main
                    class="md:col-span-2 bg-white rounded shadow p-4 flex flex-col"
                >
                    <!-- Live Video Preview -->
                    <div class="relative">
                        ...
                        <div
                            class="absolute top-2 left-2 flex items-center gap-2 px-3 py-1 bg-black/80 backdrop-blur-md border border-red-500 shadow-md rounded-full animate-fade-in"
                        >
                            <!-- Glowing Dot -->
                            <span class="relative flex h-3 w-3">
                                ...
                                ></span>
                                <span
                                    class="relative inline-flex h-3 w-3 rounded-full bg-white border-2 border-red-600"
                                ></span>
                            </span>
                            <!-- Animated Gradient Text -->
                            <span
                                class="text-sm font-semibold tracking-wide bg-gradient-to-r from-red-400 via-white to-red-600 bg-[length:200%_auto] bg-clip-text text-transparent animate-gradient-slide"
                            >
                                Live
                            </span>
                        </div>
        ...
        >
            <div
                class="bg-white p-4 rounded-lg w-full mx-4 max-w-md sm:max-w-lg md:max-w-xl relative"
            >
                <!-- Close Button -->
                <button
                ...
                <AddSchedule :selectedSession="selectedSession" />
            </div>
        </div>
    </div>
</template>
