...
export default {
    ...
    setup() {
        ...
        const logout = async () => {
            try {
                // appStore.logout() calls POST /api/logout (revokes the server-side
                // Passport token) and expires the HttpOnly cookie.
...
<template>
    <div class="flex min-h-screen">
        ...
        >
            ...
            <div
                v-if="frontLang && frontLang.lang"
                class="text-xl font-bold mb-5"
            >
                {{ frontLang.lang.uniquee }}
            </div>
            ...
            <nav>
                <ul>
                    <li
                        ...
                        :class="{
                            'bg-gray-500': currentView === item.component,
                        }"
                    >
                        <i :class="item.icon" class="mr-2"></i>
                        <span v-if="frontLang && frontLang.lang">{{
                            frontLang.lang[item.component]
                        }}</span>
                    </li>
                </ul>
            </nav>
        ...
        >
            <header
                class="bg-green-400 text-white px-4 py-3 shadow-md flex justify-between items-center w-full"
            >
                ...
                <div class="hidden md:flex items-center gap-4 ml-auto">
                    <div class="flex items-center gap-2">
                        ...
                        />
                        <button
                            class="bg-blue-500 text-white rounded-lg px-4 py-2 text-sm sm:text-base"
                        >
                            🔍
                        </button>
                    </div>
